...
from qkdrouter import QKDRouter
...
network_config = "src/star_network2.json"
...
for router in network_topo.get_nodes_by_type(RouterNetTopo.QUANTUM_ROUTER):
    if router.name == start_node_name:
        node1 = QKDRouter(router, QKDNode(router.name, router.timeline))
    elif router.name == end_node_name:
        node2 = QKDRouter(router, QKDNode(router.name, router.timeline))

nm = node1.router.network_manager
